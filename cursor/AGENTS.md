...
5. 顶层代理对应义务（禁止对已 `cancelled` 流程发起任何角色 Task）见 §4.19。
...
| QA 记录已完成，但编程规范 lint 门禁未通过（`lintPassed=false`，R15）？ | 须由 quality-assurance-engineer 运行 `lint-run.mjs` 并整改至 `gatePassed=true`；**不得发起 test-engineer 或收尾** |
| QA 记录已完成，但静态代码质量门禁未通过（`staticScanPassed=false`，R16）？ | 须由 quality-assurance-engineer 运行 `static-scan-run.mjs` 并将重复代码/安全扫描整改至均 `gatePassed=true`；**不得发起 test-engineer 或收尾** |
...
- QA 记录完成但编程规范 lint 门禁未通过（`test-results/qa/.lint-result.json` 缺失或 `gatePassed≠true`，且未满足 R15 双要素豁免）
- QA 记录完成但静态代码质量门禁未通过（`test-results/qa/.static-scan-result.json` 缺失或重复代码/安全扫描任一 `gatePassed≠true`，且未满足 R16 对应双要素豁免）
...
| 每批次 QA 通过后 | 项目经理 → 测试工程师（**批次集成测试**） | 先项目经理判定，再派发 test-engineer 对本批次做集成测试 + 批次 E2E（`--scope=batch`）+ **接口测试（R14）** + **存储对账（R17）**；判据见 §8.3（`hotfix` 模式跳过本行，见 R11） |
...
| 放行（全流程测试闭环） | `finalTestRequired && finalTestComplete && lintPassed && staticScanPassed`（R15/R16） | 全部开发+QA+批次测试（含批次 E2E）+**最终整体集成测试**（含最终 E2E）+**编程规范 lint 门禁**+**静态代码质量门禁**均通过；`hotfix` 模式下 batch 相关判据恒真（见下方 R11） |
...
| **编程规范 lint 门禁**（R15，非 docs-only） | `qaComplete && !lintPassed` | quality-assurance-engineer 运行 `lint-run.mjs`，整改至 `gatePassed=true`（机读产物 `test-results/qa/.lint-result.json`）；未通过前**不得推进测试或宣告完成** |
| **静态代码质量门禁**（R16，非 docs-only） | `qaComplete && !staticScanPassed` | quality-assurance-engineer 运行 `static-scan-run.mjs`，整改重复代码/安全扫描至均 `gatePassed=true`（机读产物 `test-results/qa/.static-scan-result.json`）；未通过前**不得推进测试或宣告完成** |
...
>
> **R15（编程规范 lint 硬门禁，唯一权威定义）**：`full`（含 `greenfield`/`feature`/`governance-overhaul`）与 `hotfix` 迭代，QA 阶段须满足：
> - 判据结构与 E2E 门禁同构（运行器写 `gatePassed` 机读产物 → lib 读入 → 门禁判定）；**执行命令与产物**：`node .cursor/scripts/lint-run.mjs` → `test-results/qa/.lint-result.json`。
> - **命令解析优先级**：`harness.config.json → qa.commands.lint` 覆盖 > 构建清单自动探测 > 栈默认（Node/Python/Go/Rust/Ruby 等有默认；Java/PHP/.NET 等无默认）；多数项目不必手配 config，仅 monorepo/自定义脚本名/探测不准时覆盖。`detail-design-spec.md` §5 由架构师填入与默认一致的留痕，不作为 Hook 输入。
> - **判据**：`lintPassed = readLintResult()?.gatePassed===true`（须有 lint 命令且退出码为 0）；`docs-only` 视为满足。QA 记录完成但 `lintPassed=false` 时 `gate-stop-workflow` 注入 followup，且**不得发起 test-engineer**（判定函数见 §10 索引）。
> - **适用性豁免**：见上表 R15 行；无默认 lint 的栈须声明等价命令或走豁免，不得静默放过。
>
> **R16（静态代码质量硬门禁：重复代码 DRY + 安全静态扫描，唯一权威定义）**：`full`（含 `greenfield`/`feature`/`governance-overhaul`）与 `hotfix` 迭代，QA 阶段须满足：
> - 判据结构与 R15 同构，但**跨技术栈通用、不做 per-stack 探测**（本框架要求 `Node.js >= 18`，两项工具均经 `npx` 直接获取）；**执行命令与产物**：`node .cursor/scripts/static-scan-run.mjs` → `test-results/qa/.static-scan-result.json`（含 `duplication`/`security` 两个子结果）。
> - **默认工具**：重复代码检测 `jscpd-rs`（`npx --yes jscpd-rs --threshold 5 --exitCode 1 ...`，5% 阈值超限退出码非 0）；安全静态扫描 `gitleaks-secret-scanner`（`npx --yes gitleaks-secret-scanner ...`，检出密钥即退出码非 0）。**命令解析优先级**：`harness.config.json → qa.commands.dupCheck`/`qa.commands.securityScan` 覆盖 > 框架默认值；多数项目不必手配 config。
> - **判据**：`staticScanPassed = (dupCheckExempt || duplication.gatePassed) && (securityScanExempt || security.gatePassed)`；`docs-only` 视为满足。QA 记录完成但 `staticScanPassed=false` 时 `gate-stop-workflow` 注入 followup，且**不得发起 test-engineer**（判定函数见 §10 索引）。
...
- **R17 门禁能力边界**：章节/表头存在性、分类型适用行、至少一条适用行、描述列非空、「其他」/「不适用」备注非空、存储介质类别关键词、批次任务包编号覆盖为机读硬门禁；对账方式是否真正查到对应介质、预期/实际是否语义正确等，仍由 QA/PM 文字审查（§8.4），Hook 不声称已验证语义。
- **约束后果**：批次 `gatePassed≠true` 时视为本批次集成测试未完成，**不得推进下一批次**；最终 `gatePassed≠true` 时**不得宣告项目完成**。
- **适用范围**：适用于 `full` 模式下的 `greenfield` / `feature` / `governance-overhaul` 及 `hotfix` 迭代（`hotfix` 按 R11 折叠为单次通道，测试严格程度不降低）；`docs-only` 豁免；无 UI 项目按 §8.2「双要素豁免机制」表 E2E 行豁免（详见 `test-engineer.md`「E2E 适用性豁免」）。
...
- **批次 + 最终 E2E 均有机读判据**（`batchE2ePassed` / `finalE2ePassed`）；**编程规范 lint 门禁**亦有机读判据（`lintPassed`，R15，读取 `test-results/qa/.lint-result.json`）；**静态代码质量门禁**亦有机读判据（`staticScanPassed`，R16，读取 `test-results/qa/.static-scan-result.json`）；**批次接口测试报告章节存在性**亦有机读判据（`batchApiReportPresent`，R14，检查「## 接口测试报告」章节非空）；**批次存储对账**亦有机读判据（`batchStorageReconPresent`，R17，检查「## 存储对账记录」非空、适用分类型行、至少一条适用行、描述列完备、「其他」/「不适用」备注、存储介质关键词与批次任务包覆盖）；**交互断言、接口用例语义正确性、存储对账查验语义、SRP/SOLID/清晰命名等语义类规范**因不可机械判定而由 QA/PM 文字审查兜底。
- **`test-results/` 受控运行产物例外**：E2E 机读结果（`test-results/e2e/.e2e-batch-result.json`、`.e2e-final-result.json`）、**编程规范 lint 机读结果**（`test-results/qa/.lint-result.json`）、**静态代码质量机读结果**（`test-results/qa/.static-scan-result.json`）、QA 运行留痕（`test-results/qa/qa-run-result.json`）及 Playwright trace/截图/video 由 `e2e-run.mjs` / `lint-run.mjs` / `static-scan-run.mjs` / `qa-run.mjs` / Playwright **进程内 `writeFileSync` 写盘**，不在 `sourceDirs` / `buildManifests` / `testConfigs` / `rootPatterns` 内，**不触发** `gate-dev-workflow`；`.gitignore` 已忽略 `test-results/`。此为**受控运行产物**，非绕过门禁；QA/测试阶段不得据此判定「脚本绕过 Hook」。
...
| R6 | `.cursor/scripts\|agents\|hooks/**` 三目录纳入机制门禁 | §8.1 Hook 一览表 |
| R8 | 禁止越级发起 Task | §4.8 |
| R9 | hotfix 开发前须校验设计存在性与 E2E 适用性，缺失经 SA 微任务补齐 | §5 `hotfix` 门禁链脚注；`workflow-gate-lib.mjs` 的 `checkHotfixDesign` |
...
| R15 | QA 须运行 lint 且 `gatePassed=true`；双要素豁免 | §8.2（唯一权威定义）；`readLintResult` / `checkLintClean` / `isLintExempt`；`lint-run.mjs` |
| R16 | QA 须运行重复代码检测+安全扫描且均 `gatePassed=true`；双要素豁免 | §8.2（唯一权威定义）；`readStaticScanResult` / `checkStaticScanClean` / `isDupCheckExempt` / `isSecurityScanExempt`；`static-scan-run.mjs` |
