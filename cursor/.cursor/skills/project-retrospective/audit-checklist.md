...
| A7 | hotfix 且 `hotfix_p0_impact: none` 时，「## 用户确认记录」含「hotfix影响面」判断依据行 | §5 R9 |
...
| B3 | 技术选型经用户确认（`## 用户确认记录` 含技术选型/技术栈确认行，R18 机读） | §5 无效成果物 |
| B4 | `design-problem-list.md` 设计审核已通过（R18：12 维+可修复字段+P0 覆盖矩阵含验收标准+审核结论通过/复审通过） | §5、R18 |
...
| D2 | 每条开发线有独立 QE 记录 | §5、§6 |
| D3 | QE 非抽样：任务包全量单元测试已运行 | §5 无效成果物 |
...
| # | 检查项 | 规约依据 |
| --- | ------ | -------- |
| E1 | 每批次 QE 后有批次集成测试行（非 hotfix） | §6、§8.3 |
...
| G2 | agent 文件 `model` slug 为系统可用模型（未回退至父 agent 模型） | §1 |
