...
| 适配目录 | 说明 | 状态 |
| -------- | ---- | ---- |
| `cursor/` | 适配 Cursor 的完整规约（自动 Hook 门禁、7 个子角色、脚本、模板、E2E 与 QA 硬门禁 R15/R16、架构/代码通用原则） | 可用 |
| `trae/` | 适配 Trae 的完整规约（手动 `gate-check` 门禁、7 个 Subagent、脚本、模板、E2E 与 QA 硬门禁 R15/R16、架构/代码通用原则） | 可用 |

> **各适配共享要点**（细节见各适配 `AGENTS.md` 与自带 README，此处仅作容器层速览）：`detail-design-spec.md` 定义架构设计原则（§2）与代码编写通用原则（§5）、安全编码要求（§8）；产品经理在设计阶段审核架构原则，系统架构师产出时须遵循；QA 阶段含 R15 编程规范 lint 与 R16 静态代码质量（重复代码 + 密钥泄露扫描）硬门禁。
