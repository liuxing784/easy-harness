...
- **接口测试（R14）**：定义与豁免判据的唯一权威源见 `test-engineer.md`「接口测试（R14）」一节。本处仅引用：批次集成测试阶段须做接口测试，测试报告须含非空「## 接口测试报告」章节；机读判据 `batchApiReportPresent` 并入 `batchTestComplete`。
...
> 规则编号（R2/R3/R5/R6/R8/R9/R10/R11/R12/R13/R14/R15/R16/B1/TG-D-4）的定义位置见各章节正文中对应编号首次出现处的说明。本索引仅作导航用，不新增约束。
