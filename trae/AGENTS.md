...
| 自检项 | 不满足时的动作 |
| ------ | -------------- |
| 本回合是否由**顶层代理亲自**写入受门禁路径（源码 / `.trae/scripts\|agents\|hooks/**` / 构建产物）？（R5） | 属越权代写，须撤销并改由对应子 agent 执行 |
| 目标流程 `process.md` 是否已 `cancelled: true`？（R10） | 禁止再对其发起任何角色 Task |
...
- `develop-task-list.md` 缺少 §3 分派方式分析
- 通过 Shell 重定向、`tee`、脚本批量写入等方式绕过 Hook
- 一个 QA Task 覆盖多条开发线，但未显式标注「批量/全量审查」并列明全部任务包编号
- 质量审核以**抽样**方式核查单元测试（须运行全量单元测试套件）

> 其余无效成果物情形（阻塞/缺分派计划/缺四件成果物/lint 未通过/静态扫描未通过/缺 E2E 结果/cancelled 流程等）已由 `gate-dev-workflow`、`gate-dev-shell`、`gate-role-sequence`、`gate-stop-workflow` 机械门禁拦截，此处不再赘述。
