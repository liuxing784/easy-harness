...
本规约由 Cursor 版本适配至 Trae。Trae 与 Cursor 的核心能力差异及降级方案如下；**其余文字规则（R2/R3/R5/R6/R8/R9/R10/R11/R12/R13/B1/TG-D-4）保持不变**。
...
- **用户交互**：Subagent 无 `AskUserQuestion` 工具；需用户确认时，Subagent 在返回结果中标注「需要用户确认：[问题]」，由顶层 Agent 代为询问，确认结果传回后继续。

### 0.2 角色推荐模型映射

Trae 会话可用模型共 4 款，能力梯度（高 → 低）为：`DeepSeek-V4-Pro`（旗舰推理+编码）> `glm-5.2`（旗舰综合）> `qwen-3.7-plus`（中等偏上）> `DeepSeek-V4-Flash`（快速轻量）。下表为各角色推荐模型（已在各角色文件 frontmatter 的 `model` 字段内 pin，见 §1 角色定义表）：

| 角色 | Cursor 原 slug | Trae 推荐 model | 能力档位 | 说明 |
| ---- | -------------- | --------------- | -------- | ---- |
| project-manager | composer-2.5 | `glm-5.2` | 旗舰综合 | 流程编排、`workflow_mode`/`iterationType` 判定、分派策略需较强综合理解 |
| requirements-analyst | claude-sonnet-5 | `qwen-3.7-plus` | 中等偏上 | 需求挖掘与确认，偏沟通+结构化文档，中等偏上足够 |
| system-architect | claude-opus-4-8 | `DeepSeek-V4-Pro` | 旗舰推理 | 架构深度推理+技术选型+详细设计，需最强模型（对标 cursor opus） |
| product-manager | composer-2.5 | `qwen-3.7-plus` | 中等偏上 | 设计审核、问题识别，需严谨判断 |
| development-engineer | gpt-5.3-codex | `glm-5.2` | 旗舰综合 | 编码实现需强编码+工程实践，glm-5.2 旗舰综合胜任 |
| quality-assurance-engineer | composer-2.5 | `qwen-3.7-plus` | 中等偏上 | 代码审查+安全审计+质量报告，需中等+严谨 |
| test-engineer | composer-2.5 | `DeepSeek-V4-Flash` | 快速轻量 | 测试执行偏流程化+模板化（vitest/端到端验证/报告填写），Flash 快速足够 |

**模型分配概览**：旗舰推理 1（SA）/ 旗舰综合 2（PM+DE）/ 中等偏上 3（RA+PDM+QA）/ 快速轻量 1（TE）。

> 推荐模型仅作能力匹配建议；实际选择可由用户在会话中调整。**禁止**以「换用更弱模型以帮助子 agent 通过门禁」为目的偏离推荐模型（见 §4.3 适配）。

### 0.3 门禁机制降级（脚本门禁 + 强制自检）

**Trae 无 Cursor 风格 Hook 自动执行**（`preToolUse` / `beforeShellExecution` / `stop` 均不被原生触发）。`.trae/hooks.json` 保留 5 个 Hook 注册声明，但**仅作门禁协议声明文件**，Trae 不会自动执行其中 `command`。

**降级方案**：以 `.trae/rules/gate-protocol.mdc`（`alwaysApply: true`）为强制入口，要求顶层代理在每次受控操作前手动调用 `node .trae/scripts/gate-check.mjs <子命令>` 并尊重其 `deny` 结果。gate 脚本（`workflow-gate-lib.mjs` + 5 个 `gate-*.mjs`）判定逻辑与 Cursor 版本完全一致，`gate-selftest` / `gate-scenarios` 全量回归通过；仅调用时机由自动改为手动。详见 `.trae/rules/gate-protocol.mdc` 与 §8。

### 0.4 工具名称映射

| Cursor | Trae | 说明 |
| ------ | ---- | ---- |
| Task（发起子 agent） | Agent（按 `description` 匹配调用对应 Subagent） | Trae 原生 Subagent，定义于 `.trae/agents/{name}.md` |
| AskQuestion | （无对应工具） | Subagent 无 `AskUserQuestion`；需用户确认时在返回结果中标注，由顶层 Agent 代为询问 |

> 正文中「Task」「发起 Task」「子 agent Task」等表述均指 Trae 的 `Agent` 工具调用。Trae 的 `Agent` 工具按 Subagent 的 `description` 匹配调用，角色定义文件位于 `.trae/agents/`。
...
| -------- | ------------- | -------------- |
| 项目经理 | project-manager | `.trae/agents/project-manager.md` |
| 需求分析师 | requirements-analyst | `.trae/agents/requirements-analyst.md` |
| 系统架构师 | system-architect | `.trae/agents/system-architect.md` |
| 产品经理 | product-manager | `.trae/agents/product-manager.md` |
| 开发工程师 | development-engineer | `.trae/agents/development-engineer.md` |
| 质量保障工程师 | quality-assurance-engineer | `.trae/agents/quality-assurance-engineer.md` |
| 测试工程师 | test-engineer | `.trae/agents/test-engineer.md` |

Trae 下角色定义文件（`.trae/agents/*.md`）为原生 Subagent，由内置 "Agent" 按 `description` 匹配调用（见 §0.1）。frontmatter 中的 `name` / `description` / `model` / `tools` 均由 Trae 自动解析：`model` 在文件内 pin（见 §0.2 映射表），`tools` 按最小权限原则限定。
...
16. **门禁调用协议（Trae 降级）**：本项目在 `.trae/hooks.json` 声明了 5 个流程门禁，但 Trae **不自动执行** Hook（见 §0.3）。顶层代理**必须**在受控操作（写入受门禁路径 / 执行受控 Shell / 分派角色 Agent / 回合收尾）前手动调用 `node .trae/scripts/gate-check.mjs <子命令>`（见 `.trae/rules/gate-protocol.mdc`）并尊重其 `deny` 结果。门禁拒绝的操作**不得**改用其他工具绕过（例如 Write 被拒后改用 Shell 写文件、gate-check `deny` 后跳过调用直接执行）。
17. **工具链安装须询问用户**：顶层代理**禁止**直接执行系统级工具链安装命令，必须交由 `development-engineer` 或 `test-engineer` 按各自文件中「检测 → 询问 → 确认 → 安装」流程执行（步骤定义以 `development-engineer.md`「依赖与环境工具链」一节为准，`test-engineer.md` 适用同一流程）；用户确认后创建 `.trae/hooks/.toolchain-install-approved.json`（默认 60 分钟有效）配合 Hook 放行。
...
| 测试工程师（批次集成测试） | 本批次开发线对应的质量审核全部通过；**本批次 E2E `gatePassed=true`**（Chromium headless 覆盖本批次 P0 子集，机读产物 `test-results/e2e/.e2e-batch-result.json`） | 无阻塞 |
...
| 每批次 QA 通过后 | 项目经理 → 测试工程师（**批次集成测试**） | 先项目经理判定，再派发 test-engineer 对本批次做集成测试 + 批次 E2E（`--scope=batch`）；判据见 §8.3（`hotfix` 模式跳过本行，见 R11） |
...
本项目通过流程门禁脚本对高风险操作做**确定性拦截**，与 §4 文字规则互补。**Trae 适配声明**：Trae 无 Cursor 风格 Hook 自动执行机制，`.trae/hooks.json` 仅作门禁协议声明文件；实际拦截由顶层代理按 `.trae/rules/gate-protocol.mdc` 协议手动调用 `node .trae/scripts/gate-check.mjs <子命令>` 实现（见 §0.3）。门禁判定逻辑（`workflow-gate-lib.mjs` + 5 个 `gate-*.mjs`）与 Cursor 版本完全一致，`gate-selftest` / `gate-scenarios` 全量回归通过；仅调用时机由「IDE 自动触发」改为「顶层代理手动调用并尊重 deny」。门禁路径与 Shell 模式以 `harness.config.json` 为默认，并与当前活跃 `docs/**/design/gated-artifacts.json`（可选，架构师维护）合并。活跃路径由 `.trae/harness-state.json` 或 `HARNESS_PROCESS_PATH` 决定。
...
| **批次集成测试**（非 hotfix） | `qaComplete && !batchTestComplete` 且处于开发阶段 | 分派 test-engineer 做**批次集成测试**（含批次 E2E） |
...
- **`gatePassed` 公式**：`gatePassed = allPassed && coverageComplete`（Chromium 覆盖全部 required P0 且无未解释 skip 且均通过）。`batchTestRowComplete` / `finalTestRowComplete` 仅反映进度行完成；`batchE2ePassed` / `finalE2ePassed` 读取对应结果文件的 `gatePassed`。`batchTestComplete = batchTestRowComplete && batchE2ePassed`；`finalTestComplete = finalTestRowComplete && finalE2ePassed`。**`hotfix` 模式下按 R11 折叠**（见 §8.2），`batchTestComplete` 恒真，`finalTestRequired` 不依赖 `batchTestComplete`。
...
- **批次 + 最终 E2E 均有机读判据**（`batchE2ePassed` / `finalE2ePassed`）；**交互断言语义粒度**因不可机械判定而由 QA/PM 文字审查兜底。
...
| R6 | `.trae/scripts\|agents\|hooks/**` 三目录纳入机制门禁；白名单豁免见 `gatedPaths.dotTraeExemptPatterns` | §8.1 Hook 一览表 |
