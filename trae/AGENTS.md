...
| 角色 | Trae 推荐 model | 能力档位 | 说明 |
| ---- | --------------- | -------- | ---- |
| project-manager | `glm-5.2` | 旗舰综合 | 流程编排、`workflow_mode`/`iterationType` 判定、分派策略需较强综合理解 |
| requirements-analyst | `qwen-3.7-plus` | 中等偏上 | 需求挖掘与确认，偏沟通+结构化文档，中等偏上足够 |
| system-architect | `DeepSeek-V4-Pro` | 旗舰推理 | 架构深度推理+技术选型+详细设计，需最强模型 |
| product-manager | `qwen-3.7-plus` | 中等偏上 | 设计审核、问题识别，需严谨判断 |
| development-engineer | `glm-5.2` | 旗舰综合 | 编码实现需强编码+工程实践，glm-5.2 旗舰综合胜任 |
| quality-assurance-engineer | `qwen-3.7-plus` | 中等偏上 | 代码审查+安全审计+质量报告，需中等+严谨 |
| test-engineer | `DeepSeek-V4-Flash` | 快速轻量 | 测试执行偏流程化+模板化（vitest/端到端验证/报告填写），Flash 快速足够 |
...
门禁方案见 `.trae/rules/gate-protocol.mdc`（`alwaysApply: true`）与 §8。
...
| AskUserQuestion | Subagent 无此工具；需用户确认时在返回结果中标注，由顶层 Agent 代为询问 |
...
12. **开发线完成后禁止直接收尾**：开发工程师 Task 返回后，**本回合须继续**调用项目经理 → 分派质量保障工程师。
13. **回合结束前自检（强制）**：
...
16. **子 agent Task 失败时必须阻塞，禁止降级自行执行**：当子 agent Task 因超时、取消或错误而未正常返回时，顶层代理须遵守以下升级策略，**严禁**以任何形式自行替代子 agent 完成其职责（包括简化 prompt 重试、直接写文件、直接执行 Shell 命令等）：
...
| 测试工程师（最终整体集成测试） | 全部任务包的开发、QA 与各批次集成测试（含各批次 E2E）均已执行完成；全量 E2E `gatePassed=true`（判据见 §8.3） | 无阻塞 |

> **两级集成测试 + E2E 机械门禁**：测试工程师执行两类集成测试——①**批次集成测试**：每批次 QA 通过后对本批次新交付任务包做集成测试，`gatePassed≠true` 时不得推进下一批次；②**最终整体集成测试**：全部任务包与各批次 E2E 闭环后对整个产品做端到端集成测试，`gatePassed≠true` 时不得宣告项目完成。`测试判定`（最终交付依据）以最终整体集成测试（含最终 E2E）结论为准。执行命令、产物路径、浏览器范围与 `gatePassed` 公式的唯一权威定义见 §8.3。
...
| 各开发线完成后 | 质量保障工程师（每条开发线/批次独立审核） |
| 每批次 QA 通过后 | 项目经理 → 测试工程师（批次集成测试，**含接口测试（R14）**） |
| 全部任务包开发+QA+各批次集成测试完成后 | 项目经理 → 测试工程师（最终整体集成测试） |
...
本项目通过流程门禁脚本对高风险操作做**确定性拦截**，与 §4 文字规则互补。`.trae/hooks.json` 仅作门禁协议声明文件；实际拦截由顶层代理按 `.trae/rules/gate-protocol.mdc` 协议手动调用 `node .trae/scripts/gate-check.mjs <子命令>` 实现（见 §0.3）。门禁判定逻辑（`workflow-gate-lib.mjs` + 5 个 `gate-*.mjs`）完整，`gate-selftest` / `gate-scenarios` 全量回归通过。门禁路径与 Shell 模式以 `harness.config.json` 为默认，并与当前活跃 `docs/**/design/gated-artifacts.json`（可选，架构师维护）合并。活跃路径由 `.trae/harness-state.json` 或 `HARNESS_PROCESS_PATH` 决定。
...
- **接口测试（R14，开发窗口批次集成测试阶段必测，唯一权威定义）**：非 hotfix 迭代（含 greenfield/feature/governance-overhaul 等），**开发窗口的批次集成测试阶段**（每批次 QA 通过后对本批次做的集成测试，**非**最终整体集成测试阶段）**必须做接口测试**，且测试报告须含**非空**的「## 接口测试报告」章节（至少一条真实表格数据行）。机读判据 `batchApiReportPresent` 由 `workflow-gate-lib.mjs` 的 `checkBatchApiTestReport()` 扫描当前活跃 docs 子树 `test/` 目录下 `*.md` 计算；缺失或为空时 `batchTestComplete=false`，`gate-stop-workflow` 注入 R14 followup，**不得推进下一批次或最终整体集成测试**。R14 仅约束批次阶段，最终整体集成测试与 hotfix 折叠通道不并入此判据。
...
- **批次 + 最终 E2E 均有机读判据**（`batchE2ePassed` / `finalE2ePassed`）；**批次接口测试报告章节存在性**亦有机读判据（`batchApiReportPresent`，R14，检查「## 接口测试报告」章节非空）；**交互断言语义粒度**因不可机械判定而由 QA/PM 文字审查兜底。
...
| R8 | 禁止越级发起 Task（不得跳过前一角色或在门禁链未满足时发起后一角色 Task） | §4.8（合并项） |
...
| R10 | 流程终止不可逆：用户确认取消后 `process.md` 写入 `cancelled: true` 即被 Hook 永久冻结，任何角色不得再修改/推进 | §3「流程终止（不可逆，R10）」；`workflow-gate-lib.mjs` 的 `isCancelledProcessFile`/`isProcessFilePath` |
