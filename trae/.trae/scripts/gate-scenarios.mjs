...
const HOOKS_DIR = path.join(PROJECT_ROOT, '.trae/hooks');
...
const CONFIRM_SECTION = [
  ...
  '| ------ | ---- | ------------ |',
  '| 需求摘要 | 2026-01-01 | 已确认 |',
  '| 技术选型 | 2026-01-01 | 确认采用 Node.js + Express 技术栈 |',
].join('\n');
...
// R18 完整格式的清洁设计问题清单：含可修复字段、需求覆盖度维度、P0 覆盖矩阵、审核结论
function buildCompleteDpl(hasUnresolved = false) {
  const dimRows = [
    ['需求覆盖度', '无', '低', '否', '', '', '', '', ''],
    ['目标达成性', '无', '低', '否', '', '', '', '', ''],
    ['功能', hasUnresolved ? '边界未定义' : '无', hasUnresolved ? '高' : '低', hasUnresolved ? '是' : '否', hasUnresolved ? '否' : '', hasUnresolved ? 'detail-design-spec.md' : '', 'R-001', 'development-engineer', hasUnresolved ? '明确各接口边界条件与异常处理' : ''],
    ['体验', '无', '低', '否', '', '', '', '', ''],
    ['可行性', '无', '低', '否', '', '', '', '', ''],
    ['MVP 范围', '无', '低', '否', '', '', '', '', ''],
    ['任务可执行性', '无', '低', '否', '', '', '', '', ''],
  ...
  const coverageMatrix = [
    ...
    '| -------- | ------ | -------- | -------- | ---------------- | ------ | -------- |',
    '| R-001 | P0 | 新增待办项功能完整实现 | 待办接口设计 | detail-design-spec.md §2 | T0-1 | 已覆盖 |',
    '',
  ].join('\n');
  const conclusion = [
    '## 审核结论',
...
const DPL_CLEAN = buildCompleteDpl(false);
const DPL_UNRESOLVED = buildCompleteDpl(true);
...
const QE_DIR = path.join(PROJECT_ROOT, 'test-results/qe');
const QE_FILES = {
  lint: path.join(QE_DIR, '.lint-result.json'),
  staticScan: path.join(QE_DIR, '.static-scan-result.json'),
};
const e2eSnapshot = {};
const qaSnapshot = {};
...
function snapshotE2e() {
  ...
  for (const [key, file] of Object.entries(QE_FILES)) {
...
function writeLintPass() {
  fs.mkdirSync(QE_DIR, { recursive: true });
  const result = {
    gatePassed: true,
    reason: 'passed',
    stack: 'node',
  ...
  fs.writeFileSync(QE_FILES.lint, `${JSON.stringify(result, null, 2)}\n`, 'utf8');
}
...
function clearLint() {
  fs.rmSync(QE_FILES.lint, { force: true });
}

function writeStaticScanResult(result) {
  fs.mkdirSync(QE_DIR, { recursive: true });
  fs.writeFileSync(QE_FILES.staticScan, `${JSON.stringify(result, null, 2)}\n`, 'utf8');
}
...
function clearStaticScan() {
  fs.rmSync(QE_FILES.staticScan, { force: true });
}
...
function greenfieldScenarios() {
  ...
  // R15/R16 场景：QE 完成但 lint/static scan 未通过，stop 门禁应 followup
  ...
  clearE2e('batch');
  clearE2e('final');
  check('G12 R15：QE 完成但 lint 门禁未通过', 'followup', {
