...
| 接口测试 | **测试工程师** | **开发窗口批次集成测试阶段必测（R14）**，覆盖本批次涉及的 HTTP/RPC/CLI 等对外接口契约（入参、出参、状态码/错误码、边界与异常路径） |
...
- **适用阶段**：**开发窗口的批次集成测试阶段**（即每批次 QA 通过后对本批次新交付任务包做的集成测试），**不是**最终整体集成测试阶段。非 hotfix 迭代（含 greenfield/feature/governance-overhaul 等）均须执行。
...
项目**无对外接口**时，R14 接口测试判据可豁免，须同时满足以下两项（与「E2E 适用性豁免」同构）：

1. `docs/**/design/gated-artifacts.json` 中已声明 `"apiTestApplicability": "n/a"`（由 system-architect 在设计阶段或热修最小设计微任务中写入）；
2. `process.md`「## 用户确认记录」含一行明确的接口测试豁免确认（行内须含「接口测试」与「豁免 / 不适用 / 无接口」等词，供 Hook 机械识别）。

两项皆满足时，`checkBatchApiTestReport` 判据被 `isApiTestExempt` 放行（`batchApiReportPresent` 视为满足）；测试报告仍须完整记录集成测试与 E2E 结果。**仅声明一项不生效**（避免单方面弱化门禁，R12）。
...
- **报告章节**：非空「## 存储对账记录」（模板：`.trae/templates/test-report.md`）；场景类型 / 描述列 / 存储介质 / 任务包覆盖要求见 §8.3。
