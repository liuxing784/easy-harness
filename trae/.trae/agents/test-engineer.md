...
- **hotfix**：R17 机读不并入 R11 折叠通道（与 R14 同）；但热修若涉及业务数据写入/存储，仍应在报告中记录存储对账结果。**P0 影响的 hotfix** 同 R14 一节所述，额外有非阻塞的「## 存储对账记录」结构化章节软性提醒（说明权威见 `.trae/harness/spec/gate-chain.md` R9 脚注第 4 条（执行权威：Hook/脚本））。
