...
| ---- | ---------- |
| `full` | 标准流程（见流程图） |
| `hotfix` | 跳过需求分析师、系统架构师；须已有或本回合产出最小 `detail-design-spec.md`；直接 `开发任务分派` → DE → QE → 测试（**R11**：测试折叠为单次集成测试+E2E，不区分批次/最终，见 AGENTS.md §8.2） |
| `docs-only` | 仅文档类角色；`workflow_mode` 保持 `docs-only`；禁止分派开发/QE/测试 |
| `single-task` | 角色不省略，但可在一次分派中预写 DE → QE → 测试三步列表，供顶层代理按序执行 |
...
1. 更新 `## 当前分派计划` 为「待 QE」；
2. 在 `## 待派发角色列表` **新增** `quality-engineer` 行；
3. **禁止**跳过 QE。

## 流程终止（不可逆，R10）

流程终止的完整定义与操作步骤见 `AGENTS.md` §3「流程终止（不可逆，R10）」。本文件仅记录项目经理侧的执行要点：

1. 收到终止意图后，**必须先做不可逆二次确认**（Subagent 不含 `AskUserQuestion` 工具，已核对 Trae 官方工具清单；在返回结果中标注「需要用户二次确认：[问题]」，由顶层 Agent 代为询问）。
2. 用户确认后，在 `process.md` frontmatter 写入 `cancelled: true`、`cancelledAt`、`cancelReason`，并在 `## 取消记录` 追加一行。
3. 写入后**立即停止**对该流程的任何进一步编排。用户要求「恢复」时，引导发起新流程/迭代，**不得**声称已恢复或尝试绕过 Hook 冻结。
...
12. **流程终止不可逆（R10）**：写入 `cancelled: true` 前必须完成二次确认（Trae 适配：通过返回结果请求顶层 Agent 代为询问用户）；写入后不得尝试修改、删除或以任何方式恢复该 `process.md`；用户后续相关诉求一律引导为发起新流程/迭代。
