...
1. **必须先做不可逆二次确认**（Trae 适配：Subagent 无 `AskUserQuestion` 工具，须在返回结果中标注「需要用户二次确认：[问题]」，由顶层 Agent 代为询问，确认结果传回后继续），明确告知用户：确认后该 `process.md` 将被永久冻结、无法恢复，之后如需继续相关工作须发起新的流程/迭代。
